...
'
' Usage:  cscript //nologo Get-SoftwareEvidence-Legacy.vbs [/noexe] [/out:DIR]
...
'   /out:DIR create the timestamped output folder inside DIR instead of on the
'            Desktop. Invoke-EvidenceCollection.ps1 sets this when it runs this
'            collector over SSH, so the evidence lands somewhere it can be
'            retrieved from and then cleaned up, rather than on the Desktop of
'            whatever account sshd logged in as. Mirrors -OutRoot on
'            Get-SoftwareEvidence.ps1 and -o on the Linux collector.
'
' Output: one timestamped folder on the current user's Desktop - or inside
' /out:DIR when one is given - containing exactly two files, in the SAME column
' layout Get-SoftwareEvidence.ps1 produces, so a mixed fleet of old and new
' machines all feed one format:
...
Dim cn, tsDisplay, tsStamp, outRoot, outDir, csvPath, sumPath, csvStream, sumStream
Dim adm, skipExe, arg
...
For Each arg In WScript.Arguments
  ...
  ' Left/Mid rather than Split(arg, ":") - a value like C:\Temp\ev contains a
  ' colon of its own and would be cut in half by a naive split.
  If LCase(Left(arg, 5)) = "/out:" Then outRoot = Mid(arg, 6)
Next
...
If Len(outRoot) = 0 Then outRoot = shell.SpecialFolders("Desktop")
Do While Len(outRoot) > 1 And Right(outRoot, 1) = "\"
  outRoot = Left(outRoot, Len(outRoot) - 1)
Loop
EnsureFolder outRoot
outDir = outRoot & "\SWEvidence_" & cn & "_" & tsStamp
...
' EnsureFolder PATH - create PATH and any missing parents. FileSystemObject's
' CreateFolder only ever creates one level, so a /out:DIR pointing somewhere
' two levels deep (which is what Invoke-EvidenceCollection.ps1 does when it
' stages a remote run) has to be walked from the top down. UNC and drive roots
' are left to GetParentFolderName, which returns "" once there is nothing left
' above the path - that is the loop's stop condition.
