...
.OUTPUTS
  One timestamped folder - on the current user's Desktop, or inside -OutRoot when one
  is given - containing exactly two files:
...
param(
  ...
  # Parent directory to create the timestamped SWEvidence_<host>_<stamp> folder in.
  # Defaults to the Desktop, which is what a hand-run collection wants.
  # Invoke-EvidenceCollection.ps1 sets this when it runs the collector over SSH, so
  # the evidence lands somewhere it can be retrieved from and then cleaned up, rather
  # than on the Desktop of whatever account sshd logged in as. Mirrors /out:DIR on
  # Get-SoftwareEvidence-Legacy.vbs and -o on the Linux collector.
  #
  # Named -OutRoot and not -OutDir on purpose: PowerShell variable names are
  # case-insensitive, so a parameter named -OutDir would be the same variable as the
  # $outDir this script builds below, and would silently overwrite it.
  [string]$OutRoot = ''
)
...
$root    = $OutRoot
if (-not $root) { $root = Join-Path $env:USERPROFILE 'Desktop' }
$outDir  = Join-Path $root "SWEvidence_${cn}_$stamp"
